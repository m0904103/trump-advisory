...
async function main() {
  try {
    ...
    const twMatch = content.match(/vixVersion'\) !== '[^']+'\) return '(\d+\.\d+)';/);
    let currentTwVix = twMatch ? parseFloat(twMatch[1]) : 24.20;
    ...
    // Replace TW VIX default values if updated
    ...
    console.error('Auto-update script error:', err);
  }
}
